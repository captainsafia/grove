...
describe("self-update validation", () => {
  ...
  describe("command construction with argument arrays", () => {
    ...
    test("should construct command for latest version (no args)", () => {
      const cmd = constructInstallCommand(undefined, undefined);
      expect(cmd).toBe("curl -fsSL https://safia.rocks/grove/install.sh | sh");
    });

    test("should construct command with version", () => {
      const cmd = constructInstallCommand("1.0.0", undefined);
      expect(cmd).toBe("curl -fsSL https://safia.rocks/grove/install.sh | sh -s -- v1.0.0");
    });

    test("should construct command with version already having v prefix", () => {
      const cmd = constructInstallCommand("v1.0.0", undefined);
      expect(cmd).toBe("curl -fsSL https://safia.rocks/grove/install.sh | sh -s -- v1.0.0");
    });

    test("should construct command with PR number", () => {
      const cmd = constructInstallCommand(undefined, "123");
      expect(cmd).toBe("curl -fsSL https://safia.rocks/grove/install.sh | sh -s -- --pr 123");
    });

    test("should build command from argument arrays without string interpolation", () => {
      // This test verifies that the command is built from arrays, not string interpolation
      const cmd = constructInstallCommand("2.5.1", undefined);
      // With array construction, the version is treated as a plain argument
      expect(cmd).toContain("v2.5.1");
      // Verify the command structure matches array-based construction
      expect(cmd).toBe("curl -fsSL https://safia.rocks/grove/install.sh | sh -s -- v2.5.1");
    });
  });
});
